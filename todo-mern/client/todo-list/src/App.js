...
function App() {
  ...
  const addItem = async (e) => {
    e.preventDefault();
    try{
      const res = await axios.post('http://localhost:8000/api/items', {item: itemText})
  ...
  const updateItem = async (e) => {
    e.preventDefault()
    try{
      const res = await axios.put(`http://localhost:8000/api/items/${isUpdating}`, {item: updateItemText})
